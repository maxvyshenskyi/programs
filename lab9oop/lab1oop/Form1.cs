...
namespace lab1oop
{
    public partial class Form1 : Form
    {
        ...
        {
            string name = string.IsNullOrWhiteSpace(nametext.Text) ? "Невідомий двигун" : nametext.Text;
            string type = string.IsNullOrWhiteSpace(typetext.Text) ? "Невідомо" : typetext.Text;
            int power = 0;
            int.TryParse(powertext.Text, out power);
            int phases = 0;
            int.TryParse(phasestext.Text, out phases);
            double mass = 0;
            double.TryParse(masstext.Text, out mass);
            int voltage = 0;
            int.TryParse(voltagetext.Text, out voltage);
            int efficiency = 0;
            int.TryParse(efficiencytext.Text, out efficiency);
            EMotor newMotor = new EMotor(name, type, power, phases, mass, voltage, efficiency);
            Main.Motors.Add(newMotor);
            UpdateComboBox();

            MessageBox.Show($"Новий двигун \"{name}\" додано у список!");
        }
        ...
        private void buttonremove_Click(object sender, EventArgs e)
        {
            ...
            if (index >= 0)
            {
                var motorName = Main.Motors[index].Name;
                Main.Motors.RemoveAt(index);
                MessageBox.Show($"Двигун \"{motorName}\" видалено зі списку!");
                UpdateComboBox();
            }
            else
            {
                MessageBox.Show("Оберіть двигун для видалення!");
            }
        }
